...
int matrix4_inverse(Value th) {
	...
	Mat4 *source = self;
	if (getTop(th)>1 && isCDataType(getLocal(th,1), PegMat4))
		source = (Mat4*) toHeader(getLocal(th,1));
...
int matrix4_mult(Value th) {
	if (getTop(th)<2 || !isCDataType(getLocal(th,1), PegMat4))
		return 0;
	Mat4 *self = (Mat4*) toHeader(getLocal(th,0));
	Mat4 *mat1 = (Mat4*) toHeader(getLocal(th,1));
	if (getTop(th)>2 && isCDataType(getLocal(th,2), PegMat4)) {
		Mat4 *mat2 = (Mat4*) toHeader(getLocal(th,2));
